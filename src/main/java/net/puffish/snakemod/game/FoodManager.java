...
public class FoodManager {
	...
	private FoodManager(ServerLevel world, List<Vec3> positions, int minCount, Random random) {
		this.world = world;
	...
	public static FoodManager create(ServerLevel world, SnakeMap map, Random random, float density) {
		...
		var positions = regions.stream().flatMap(region -> {
			...
			if(shape.toAabbs().size() == 1){
				var box = shape.toAabbs().getFirst();
	...
	private boolean checkCollisions(SnakeFoodEntity entity, Collection<SnakePlayer> snakes) {
		...
		for (var snake : snakes) {
			...
			if (entity.getCenter().distanceToSqr(pos) < minSquaredDistance) {
				...
				world.playSound(null, pos.x, pos.y, pos.z, SoundEvents.PLAYER_BURP, SoundSource.PLAYERS, 1.0f, random.nextFloat() * 0.1f + 0.9f);
